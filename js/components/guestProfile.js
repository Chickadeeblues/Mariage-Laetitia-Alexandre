          ...
          <div class="card">
            ...
            <div style="margin-top:15px; font-size:14px; line-height:1.6;">
              <p><strong>Régime alimentaire :</strong>
                ${guest.diet && guest.diet.length > 0
                  ? guest.diet.map(d => {
              ...
              ${guest.allergyDetails
                ? `<p style="font-size:13px; color:#c62828; font-style:italic;">Note : ${guest.allergyDetails}</p>`
                : ''}

              <div style="height:1px; background:#f5f2eb; margin:12px 0;"></div>

              <p><strong>Transport choisi :</strong>
                ${guest.transport?.mode === 'car'   ? '🚗 En voiture'
                : guest.transport?.mode === 'train' ? '🚆 En train'
                : '✈️ Autre'}
              </p>
              ...
              ` : guest.transport?.carpoolRole === 'need' ? `
                <p style="color:#b8860b; font-size:13px;">🙋 Vous recherchez ${guest.transport.seatsNeeded} places.</p>
